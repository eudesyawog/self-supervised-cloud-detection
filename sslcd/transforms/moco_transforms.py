...
from ..transforms.normalize import Sentinel1Normalize, Sentinel2Normalize, IGBP2DFCNormalize
...
class MocoLandsatPacker:
    ...
    def __call__(
        ...
        # This image augmentation library is a pain to work with
        img = to_albumentations(image)

        # Apply the geo transforms
        augmented_q = self.geo_transform(image=img)
        augmented_k = self.geo_transform(image=img)

        # Apply the radiometric transforms
        augmented_q["image"] = self.radio_transform(image=augmented_q["image"])["image"]
        augmented_k["image"] = self.radio_transform(image=augmented_k["image"])["image"]

        return (augmented_q["image"], augmented_k["image"]), label
